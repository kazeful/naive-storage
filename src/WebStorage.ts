...
export class WebStorage {
  ...
  private _getInputData<T>(value: T, options: StorageOptions) {
    const _value = {
      value,
  ...
  private _getOutputData<T>(value: string) {
    const _value = JSON.parse(value) as StorageValue<T>
  ...
  private _isExpired<T>(_key: string, _value: StorageValue<T>) {
  ...
  private _isOnce<T>(_key: string, _value: StorageValue<T>) {
